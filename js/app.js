...
let slider_filter = new Swiper('.filter-travel__swiper', {
	observer: true,
	observeParents: true,
...
const sliderTrav = document.querySelector('.travel__swiper');
let sliderData = sliderTrav.dataset.mobile;

let sliderTravel;

function mobileSlider() {
	if (window.innerWidth < sliderData && sliderTrav.dataset.swiper == 'false') {
		sliderTravel = new Swiper('.travel__swiper', {
			observer: true,
			observeParents: true,
			observeSlideChildren: true,
			slidesPerView: 1,
		...
		sliderTrav.dataset.swiper = 'true';
	}

	if (window.innerWidth >= sliderData) {
		sliderTrav.dataset.swiper = 'false';
		if (sliderTrav.classList.contains('swiper-container-initialized'))
			sliderTravel.destroy();
	}
}
mobileSlider();
...
let slider_special = new Swiper('.special__slider', {
	/*
	effect: 'fade',
	autoplay: {
		delay: 3000,
...
let slider_feedback = new Swiper('.feedback__swiper', {
	/*
	effect: 'fade',
	autoplay: {
		delay: 3000,
...
let subIconMenu = document.querySelector(".icon_sub-menu");
if (iconMenu || subIconMenu != null) {
	let delay = 500;
	let menuBody = document.querySelector(".menu__body");
	let subMenuBody = document.querySelector(".sub-menu__body");
	iconMenu.addEventListener("click", function (e) {
		if (unlock) {
			body_lock(delay);
			iconMenu.classList.toggle("_active");
			menuBody.classList.toggle("_active");
			subIconMenu.classList.remove("_active");
			subMenuBody.classList.remove("_active");
		}
	});
	subIconMenu.addEventListener("click", function (e) {
		if (unlock) {
			body_lock(delay);
			subIconMenu.classList.toggle("_active");
...
window.onload = function () {
	...
	function documentActions(e) {
		...
		if (window.innerWidth > 768 && isMobile.any()) {
			if (targetElement.classList.contains('sub-menu__arrow')) {
				targetElement.closest('.sub-menu__item').classList.toggle('_hover');
			}
			if (!targetElement.closest('.sub-menu__item') && document.querySelectorAll('.sub-menu__item._hover').length > 0) {
				_removeClasses(document.querySelectorAll('.sub-menu__item._hover'), "_hover");
			}
...
function app() {
	...
	const cards = document.querySelectorAll('.travel__column')
	function filter(category, items) {
		items.forEach((item) => {
			const isItemFiltered = !item.classList.contains(category)
			const isShowAll = category.toLowerCase() === 'all'
			if (isItemFiltered && !isShowAll) {
				item.classList.add('_anime')
			} else {
	...
	buttons.forEach((button) => {
		button.addEventListener('click', event => {
			...
			if (target.classList.contains('filter')) {
				buttons.forEach(listItem => listItem.classList.remove('filter-active'))
	...
	cards.forEach((card) => {
		card.ontransitionend = function () {
			if (card.classList.contains('_anime')) {
				card.classList.add('_hide')
				card.classList.remove('swiper-slide')
				card.classList.remove('swiper-slide-next')
...
//RANGE
const priceSlider = document.querySelector('.range-search__price');
if (priceSlider) {

	let textFrom = priceSlider.getAttribute('data-from');
	let textTo = priceSlider.getAttribute('data-to');
	let textPost = priceSlider.getAttribute('data-post');

	noUiSlider.create(priceSlider, {
		start: [2, 120],
		connect: true,
		tooltips: [wNumb({ decimals: 0, prefix: textFrom + ' ', postfix: ' ' + textPost }), wNumb({ decimals: 0, prefix: textTo + ' ', postfix: ' ' + textPost })],
		range: {
			'min': [2],
			'max': [120]
		}
